...
def index():
    ...
    if session.get("login"):
        ...
        return render_template(
            "index.tmpl", username=username, memos=memos, users=users, totp=totp
        )
    # ログインしてない場合はログインページへ
    else:
...
def login():
    ...
    if len(result) == 0:
        ...
        print(result[0]["totp"])
        totp = pyotp.TOTP(result[0]["totp"])
        print(totp.verify(posttotp))
        if totp.verify(posttotp):
            # セッション(cookieに保存)にログイン状態とユーザー名を保存
            session["login"] = True
            session["username"] = username
            session["totp"] = result[0]["totp"]
            return redirect("/")
        else:
            return render_template("login.tmpl", fail=1)
...
def register():
    ...
    if len(result) == 0:
        ...
        result = ExecuteQuery(
            "insert into users (username, password, totp) values ('{}', '{}', '{}')".format(
